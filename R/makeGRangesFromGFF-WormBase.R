...
.detectWormBaseGTFGarbage <- function(object, idCol) {
    idCol <- match.arg(idCol, choices = c("gene_id", "transcript_id"))
    ids <- mcols(object)[[idCol]]
    garbage <- ids[grepl(pattern = ":", x = ids)]
    if (hasLength(garbage)) {
        warning(paste0(
            length(garbage), " malformed ", idCol, ": ",
...
.makeGenesFromWormBaseGTF <- function(object) {
    ...
    .detectWormBaseGTFGarbage(object, idCol = "gene_id")
    object
}
...
.makeTranscriptsFromWormBaseGTF <- function(object) {
    ...
    .detectWormBaseGTFGarbage(object, idCol = "transcript_id")
    object
}
