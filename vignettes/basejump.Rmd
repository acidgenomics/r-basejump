...
grch38 <- makeGRangesFromGFF(gtf_file)
```
...
grch38 <- makeGRangesFromGFF(gff_file)
```
...
```{r}
mgi2ensembl <- mgi2ensembl(.test = TRUE)
summary(mgi2ensembl)
```
...
```{r}
panther <- panther(organism = "Homo sapiens", .test = TRUE)
summary(panther)
```
...
```{r}
url <- file.path(basejumpCacheURL, "mn.rda")
loadRemoteData(url)
```
